...
export class NoteService {
  ...
  .pipe(map(notes => notes.map(fromObjectToNote)))

  getUserNotes = () => this.localStorageService.getUserNotes()
  .pipe(map(notes => notes.map(fromOutNoteToNote)))

  getNotes = () => combineLatest([this.getOthersNotes(), this.getUserNotes()]).pipe(
